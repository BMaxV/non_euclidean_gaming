...
class RoamingRalphDemo(ShowBase):
    ...
    def non_euclidean_visible(self,Task):
        
        x,y,z=self.ralph.getHpr() # get the rotation in degrees
        
        rotations=x/360 # divide by 360, so we get rotations as a float
        
        rotations=round(rotations%2,0) 
        
        # round to 0 decimals, we care about full rotations, no in betweens
        
        # on uneven rotation numbers, hide
        
        if rotations==1:
            self.non_euclidean_object.hide()
        
        # on even rotation numbers, show
        
        if rotations in [2,0]:
            self.non_euclidean_object.show()
            
        # to transer this to any other scene/topic,
        # simply don't take ralphs rotation,
        # but instead calculate the angular position of ralph to 
        # some object
        # and count the rotations. (which is a bit more complicated)
        
        # you will have to use get_radians_from_vector 
        # and compare new and old radians values and decide when
        # a rotation has happened, because the values went from 
        # "nearly 2*pi" to "nearly 0" and vice versa.
        
        return Task.cont
    
    
    def non_euclidean_init(self,ralphStartPos):
        cube = self.loader.loadModel("BlendFile.gltf")
        cube.reparentTo(render)
        cube.setScale(0.1,0.1,0.1)
        self.non_euclidean_object=cube
        
        cube.setPos(ralphStartPos)
        taskMgr.add(self.non_euclidean_visible, "non_euclidean_task")
    ...
    def __init__(self):
        ...
        self.non_euclidean_init(ralphStartPos)
